...
**Purpose**: Agent can fully automate Pot War on BNB Chain: fetch state (HTTP), place bid (ethers.js on-chain tx), record activity (HTTP). Requires `AGENT_PRIVATE_KEY` in env.
...
- Bidding game on **BNB Chain Mainnet**. Bid BNB; **last bidder when timer ends wins 80%** of the pot.
...
### Step 1: Fetch current state (HTTP)
...
- `pot.minimumNextBid` – min bid in BNB (string, e.g. `"0.001234"`)
...
- `amountBnb` – string ≥ `pot.minimumNextBid`
- `valueWei` – `ethers.parseEther(amountBnb)`

**Output:** `txHash`, `bidder`, `amountWei`, `amountBnb` for Step 3.
